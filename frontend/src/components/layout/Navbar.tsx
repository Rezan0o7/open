...
export function Navbar() {
  ...
  React.useEffect(() => {
    ...
    setMe(auth.tokens() ? "Signed in" : null);
  }, []);
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      <div className="mx-auto max-w-[1400px] px-4">
        <div className="glass mt-3 rounded-2xl px-4 py-3 flex items-center gap-3 shadow-sm">
              ...
              <div className="absolute left-0 right-0 mt-2 glass rounded-2xl p-2">
                ...
                <div className="space-y-1">
                  {suggest.slice(0, 12).map((x, idx) => (
                    ...
                    >
                      <img
                        src={x.image || "https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&w=80&q=60"}
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-[12px] truncate" style={{ color: "var(--muted)" }}>
                          {x.type === "user" ? "Channel" : "Category"} {x.subtitle ? `• ${x.subtitle}` : ""}
                        </div>
                      </div>
                    </button>
                  ))}
          ...
          <div className="relative">
            ...
            >
              <User size={16} />
              <span className="text-sm">{me ? "Account" : "Guest"}</span>
            </button>
            ...
            {open && (
              <div className="absolute right-0 mt-2 w-56 rounded-2xl glass p-2">
                {!me ? (
                  <>
                    <a className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm hover:opacity-90" href="#/register">
                      <LogIn size={16} />
                      Register / Sign In
                    </a>
                  ...
                  <>
                    <a className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm hover:opacity-90" href="#/dashboard">
                      <LayoutDashboard size={16} />
                      Dashboard
                    </a>
                    <button className="w-full text-left flex items-center gap-2 rounded-xl px-3 py-2 text-sm hover:opacity-90" onClick={logout}>
                      <LogIn size={16} />
                      Logout
                    </button>
                  </>
